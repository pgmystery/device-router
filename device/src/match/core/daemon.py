	...
	def run(self):
		if self.match.access_token:
			while self.runnning:
				...
				r = self.match.messenger.post(self.match.lighter_url + "/hardware/online/", {"access_token": self.match.access_token})
				if r.status_code != 200:
					if self.offline == False:
						self.offline = True
						self.match.logger.log("POST to \"" + r.url + "\" was NOT working: status_code: " + str(r.status_code), "error")
				else:
